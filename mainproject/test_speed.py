...
def search():
    ...
    b1 = Button(window3, text='Show Details', width=15, bg='#57a1f8', command=lambda: my_details(t1.get('1.0', END)))
...
def signin():
    ...
    b = Button(window2, text="signup", command=sining).grid(row=4)
...
def login():
    ...
    b=Button(window1,text="login",command=logging).grid(row=3)
...
def mains():
    ...
    menubar.add_command(label="New User",command=signin)
